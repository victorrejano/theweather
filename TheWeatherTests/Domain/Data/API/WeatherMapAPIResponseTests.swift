//
//  WeatherMapAPIResponseTests.swift
//  TheWeatherTests
//
...
class WeatherMapAPIResponseTests: XCTestCase {
    var jsonData: Data!
    lazy var encoder: JSONEncoder! = { JSONEncoder() }()
    lazy var decoder: JSONDecoder! = { JSONDecoder() }()
    
    let resourceName: String = "weatherMapApiResponse"
    ...
    func testDecodeImpl() {
        let decodedObject = try! decoder.decode(WeatherMapAPIResponse.self, from: jsonData)
