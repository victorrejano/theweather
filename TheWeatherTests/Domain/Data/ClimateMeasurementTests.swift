...
class ClimateMeasurementTests: XCTestCase {
    var jsonData: Data!
    lazy var encoder: JSONEncoder! = { JSONEncoder() }()
    lazy var decoder: JSONDecoder! = { JSONDecoder() }()
    
    let resourceName: String = "climateMeasurement"
    ...
    func testDecodeImpl() {
        let decodedObject = try! decoder.decode(ClimateMeasurement.self, from: jsonData)
